        ...
        if response.status_code == 200 or response.status_code == 201:
            ...
            for record in data["records"]:
                ...
                if record["fields"].get("Pickup Status") != 99:
                    print(
                        f"The value at the {station} cell of the current order is: {record['fields'].get(station)}"
                    )
    ...
    def changeValue(self, station, value):
        ...
        if response.status_code == 200 or response.status_code == 201:
            ...
            for record in data["records"]:
                if record["fields"].get("Pickup Status") != 99:
                    print(
                        f"Current Order is: {record.get('id')}, Order Name: {record['fields'].get('Order Name')}"
                    )
